...
$sql = "
    ...
    INNER JOIN status_verifikasi sv
...
<head>
    ...
    <title>Volume per Jenis Bantuan - Monitoring CPCL</title>
...
<div class="container py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-2">
        <div>
            <h3 class="section-title mb-1">Total Volume per Jenis Bantuan</h3>
            <p class="text-muted mb-0">Rekap total volume dan satuan yang sudah terinput pada setiap jenis bantuan.</p>
        </div>
        <div class="d-flex flex-wrap gap-2">
            ...
            <a href="<?= base_url('dashboard_volume_provinsi_jenis.php') ?>" class="btn btn-outline-success">Volume per Provinsi</a>
            <a href="<?= base_url('export_volume_jenis_bantuan.php?' . http_build_query([
    ...
    <div class="card rekap-card mb-4">
        <div class="card-body">
            <form method="GET" action="<?= base_url('dashboard_volume_jenis_bantuan.php') ?>" class="row g-2 align-items-end">
                <div class="col-md-4">
                ...
                <div class="col-md-2 d-flex gap-2">
                    ...
                    <a href="<?= base_url('dashboard_volume_jenis_bantuan.php') ?>" class="btn btn-outline-secondary">Reset</a>
                </div>
            </form>
        </div>
    </div>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($data as $index => $row): ?>
                                <?php
                                $detailUrl = base_url('dashboard_volume_jenis_bantuan_detail.php?' . http_build_query([
                                    'id_jenis_bantuan' => $row['id_jenis_bantuan'],
                                    'satuan' => $row['satuan'],
                                    'status_verifikasi' => $status,
                                ]));
                                ?>
                                <tr class="clickable-row" data-href="<?= e($detailUrl) ?>">
                                    <td><?= $index + 1 ?></td>
                                    <td class="jenis-name"><?= e($row['nama_jenis_bantuan']) ?></td>
                                    <td>
