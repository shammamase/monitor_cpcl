...
$oldInput = $_SESSION['old_input_stok'] ?? [];
unset($_SESSION['error_message_upbs'], $_SESSION['old_input_stok']);

$oldHeader = [
    'id_upbs'            => $oldInput['id_upbs'] ?? '',
    'tanggal_laporan'    => $oldInput['tanggal_laporan'] ?? date('Y-m-d'),
    'periode_text'       => $oldInput['periode_text'] ?? '',
    'nama_petugas_input' => $oldInput['nama_petugas_input'] ?? ($userUpbs['nama_lengkap'] ?? ''),
    'keterangan'         => $oldInput['keterangan'] ?? '',
];
$oldDetail = array_values($oldInput['detail'] ?? []);

$pageTitle = 'Tambah Input Stok - UPBS';
...
<style>
    ...
    .detail-table td:nth-child(2) {
        min-width: 170px; /* Komoditas */
    }

    .detail-table th:nth-child(3),
    .detail-table td:nth-child(3) {
        min-width: 190px; /* Varietas / Galur */
    }

    .detail-table th:nth-child(4),
    .detail-table td:nth-child(4) {
        min-width: 150px; /* Kelas Benih */
    }

    .detail-table th:nth-child(5),
    .detail-table td:nth-child(5) {
        min-width: 170px; /* Benih Sumber */
    }

    .detail-table th:nth-child(6),
    .detail-table td:nth-child(6) {
        min-width: 150px; /* Stok Tersedia */
    }

    .detail-table th:nth-child(7),
    .detail-table td:nth-child(7) {
        min-width: 120px; /* Satuan */
    }

    .detail-table th:nth-child(8),
    .detail-table td:nth-child(8) {
        min-width: 130px; /* Harga */
    }

    .detail-table th:nth-child(9),
    .detail-table td:nth-child(9) {
        min-width: 200px; /* Keterangan Harga */
    }
...
<form action="<?= base_url('page_upbs/input_stok/store.php') ?>" method="POST" id="formInputStok">
    <div class="card section-card mb-4">
        <div class="card-body">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mb-4">
                <div>
                    <h4 class="section-title">Tambah Input Stok</h4>
                    <p class="section-subtitle">Isi informasi laporan dan tambahkan detail stok per item.</p>
                </div>
            ...
            <?php if (empty($upbsList)): ?>
                <div class="alert alert-warning mb-0">
                    Belum ada data UPBS aktif untuk satker Anda. Silakan lengkapi data UPBS terlebih dahulu sebelum melakukan input stok.
                </div>
            <?php endif; ?>
            ...
            <div class="row g-3">
                ...
                <div class="col-lg-6">
                    ...
                    <input type="text" name="nama_petugas_input" class="form-control" value="<?= e($oldHeader['nama_petugas_input']) ?>">
                </div>

                <div class="col-lg-4">
                    <label class="form-label">UPBS</label>
                    <select name="id_upbs" class="form-select" <?= empty($upbsList) ? 'disabled' : '' ?> required>
                        <option value="">-- Pilih UPBS --</option>
                        <?php foreach ($upbsList as $u): ?>
                            <option value="<?= $u['id_upbs'] ?>" <?= ($oldHeader['id_upbs'] == $u['id_upbs']) ? 'selected' : '' ?>>
                                <?= e($u['nama_upbs']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-lg-4">
                    ...
                    <input type="date" name="tanggal_laporan" class="form-control" value="<?= e($oldHeader['tanggal_laporan']) ?>" required>
                </div>

                <div class="col-lg-4">
                    <label class="form-label">Periode Teks</label>
                    <input type="text" name="periode_text" class="form-control" placeholder="Contoh: Maret 2026" value="<?= e($oldHeader['periode_text']) ?>">
                </div>

                <div class="col-12">
                    <label class="form-label">Keterangan</label>
                    <textarea name="keterangan" class="form-control" rows="2" placeholder="Keterangan tambahan jika ada"><?= e($oldHeader['keterangan']) ?></textarea>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mb-3">
                <div>
                    ...
                    <p class="section-subtitle">Tambahkan item stok sesuai komoditas, varietas/galur, dan satuannya.</p>
                </div>
            ...
            <div class="d-flex flex-wrap gap-2">
                <button type="button" class="btn btn-outline-primary" id="btnSimpanDraft">Simpan Draft</button>
                <button type="button" class="btn btn-success" id="btnSimpanFinal">Simpan Final</button>
                <a href="<?= base_url('page_upbs/input_stok/index.php') ?>" class="btn btn-secondary">Batal</a>
            </div>
        </div>
    </div>
</form>
...
const oldDetailData = <?= json_encode($oldDetail, JSON_UNESCAPED_UNICODE) ?>;
...
document.getElementById('btnSimpanDraft').addEventListener('click', function() {
    alert('Mode simpan draft belum tersedia karena tabel laporan stok belum memiliki kolom status draft/final.');
});

document.getElementById('btnSimpanFinal').addEventListener('click', function() {
    document.getElementById('formInputStok').submit();
});
