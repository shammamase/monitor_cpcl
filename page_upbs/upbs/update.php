...
if ($id_satker <= 0 || $id_upbs <= 0) {
    $_SESSION['error_message_upbs'] = 'Data UPBS tidak valid.';
...
$stmtCheck = $pdo->prepare("
    ...
    WHERE id_satker = ?
      ...
      AND id_upbs != ?
");
$stmtCheck->execute([$id_satker, $nama_upbs, $id_upbs]);
$exists = (int)$stmtCheck->fetch(PDO::FETCH_ASSOC)['total'];

if ($exists > 0) {
    $_SESSION['error_message_upbs'] = 'Nama UPBS sudah digunakan pada satker Anda.';
    header('Location: ' . base_url('page_upbs/upbs/index.php'));
    exit;
}
...
$stmt = $pdo->prepare("
    ...
    SET
        kode_upbs = :kode_upbs,
        nama_upbs = :nama_upbs,
        jenis_upbs = :jenis_upbs,
        no_hp_pengelola = :no_hp_pengelola,
        is_active = :is_active,
        updated_at = NOW()
    WHERE id_upbs = :id_upbs
...
$_SESSION['success_message_upbs'] = 'Data UPBS berhasil diperbarui.';
header('Location: ' . base_url('page_upbs/upbs/index.php'));
exit;