...
foreach ($data as &$row) {
    ...
    if (!empty($row['nama_eselon']) && $row['nama_eselon'] !== '-') {
        $totalEselon[$row['nama_eselon']] = true;
    }
...
<head>
    ...
    <title>Volume per Sumber Es.1 - Monitoring CPCL</title>
...
<div class="container py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-2">
        <div>
            <h3 class="section-title mb-1">Rekap Volume Berdasarkan Sumber Bantuan Es.1</h3>
            <p class="text-muted mb-0">Total volume bantuan dirinci per Eselon 1, sumber bantuan, jenis bantuan, dan satuan.</p>
        </div>
        <div class="d-flex flex-wrap gap-2">
            ...
            <a href="<?= base_url('export_volume_sumber_bantuan_eselon.php?' . http_build_query([
                'id_sumber' => $id_sumber,
                'eselon_1' => $eselon_1,
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="card summary-card h-100" style="background: linear-gradient(135deg, #198754, #157347);">
                <div class="card-body">
                    <div class="label">Eselon 1 Tampil</div>
                    <div class="value"><?= number_format(count($totalEselon)) ?></div>
                </div>
            </div>
        </div>
        <div class="col-md-4">
    ...
    <div class="card rekap-card mb-4">
        <div class="card-body">
            <form method="GET" action="<?= base_url('dashboard_volume_sumber_bantuan_eselon.php') ?>" class="row g-2 align-items-end">
                <div class="col-md-4">
                    <label class="form-label">Eselon 1</label>
                ...
                <div class="col-md-2 d-flex gap-2">
                    ...
                    <a href="<?= base_url('dashboard_volume_sumber_bantuan_eselon.php') ?>" class="btn btn-outline-secondary">Reset</a>
                </div>
            </form>
        </div>
    </div>
                ...
                <table class="table table-bordered table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <?php if (empty($displayGroups)): ?>
                            <tr>
                                <td colspan="6" class="text-center text-muted py-4">Belum ada data volume sesuai filter.</td>
                            </tr>
                        <?php else: ?>
                                ...
                                <?php foreach ($group['rows'] as $rowIndex => $row): ?>
                                    <tr>
                                        <?php if ($rowIndex === 0): ?>
                                            ...
                                            <td rowspan="<?= $rowspan ?>">
                                                <?php if (!empty($group['nama_eselon']) && $group['nama_eselon'] !== '-'): ?>
                                                    <a href="<?= base_url('index.php?' . http_build_query([
                                                        'eselon_1' => $group['nama_eselon'],
                                                        'status_filter' => $status,
                                                    ])) ?>" class="eselon-name text-decoration-none">
                                                        <?= e($group['nama_eselon']) ?>
                                                    </a>
                                                <?php else: ?>
                                        ...
                                        <td>
                                            <a href="<?= base_url('index.php?' . http_build_query([
                                                ...
                                                <?= !empty($row['nama_sumber']) ? e($row['nama_sumber']) : '-' ?>
                                            </a>
                                        </td>
                                        <td>
